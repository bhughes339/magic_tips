...
## Pad external ADM account number with zeroes

Oftentimes, site and specialists will provide the shorthand version of an ADM account number, which is difficult to work with internally. This will automatically pad the account number with the appropriate number of zeroes for use in the $G utility (specifically, :AAAI):
